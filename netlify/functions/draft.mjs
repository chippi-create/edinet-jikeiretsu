...
//
// 渡すのは会社の開示内容と数値、それに検討している調達手法の名前だけ。
// **証券会社名・部署名・割当先・担当者は送らないし、書かせない。**
// 「ある会社についてどの手法を検討しているか」までは残るが、
// 誰が検討しているかは残らない。そこが境界線。
//
...
const SLOTS = {
  ...
  whyMethod: {
    label: "その手法を選ぶ理由",
    spec: "検討している手法が、ほかの手法と比べてこの会社の状況に合う理由を3点。各50〜90字。" +
          "準備期間・機動性・調達額の確定の仕方・希薄化の制御といった" +
          "「仕組みの違い」で説明する。ほかの手法を否定しない。",
  },
  points: {
    label: "資本政策を検討する背景",
...
・**株価や業績の見通しについての推奨は書かない。** 事実と、そこから直接言えることだけを書く。
・**金融機関の名前を書かない。** どこが取り扱うかには触れない。

調達手法について書くときに守ること:
・手法の比較は**仕組みの違い**として書く。準備期間、資金が入る時期、
  調達額が確定するタイミング、希薄化を設計できるかどうか、といった観点で書く。
・**発行会社の信用力や、審査に通るかどうかについて一切書かない。**
  「引受審査が通らない」「審査に耐えられない」「業績が悪いので公募は難しい」
  のような表現は、遠回しなものも含めて禁止です。
  準備期間の長短として書くことはできますが、通る通らないの話にしてはいけません。
・**ほかの手法を否定しない。** どれも成立する前提で、この会社の状況に対する
  相性の違いとして書く。
...
export default async (req) => {
  ...
  // 検討している調達手法。手法名だけで、誰が扱うかは受け取らない。
  const METHODS = ["MSワラント", "公募増資", "第三者割当増資", "自社株買い", "売出し"];
  ...
  const key = `${code}-${method || "-"}-${want.slice().sort().join(",")}-${hash(material)}`;
